...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'tutor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<StartupInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/startups` : null),
    () => getStartups({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Startup',
      path: isTenantUser ? `/startups/view/${data?.[0]?.id}` : '/startups',
      entity: 'startup',
      service: AccessServiceEnum.PROJECT,
      icon: FiBox,
    },
    { name: 'Booking', path: '/bookings', entity: 'booking', service: AccessServiceEnum.PROJECT, icon: FiBox },
    { name: 'Tutor', path: '/tutors', entity: 'tutor', service: AccessServiceEnum.PROJECT, icon: FiBox },
